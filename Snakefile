...
DATADIR="/home/arthur/Bureau/postdocs/isem/phylo/data"
#Path to tools directory
TOOLDIR="/home/arthur/Bureau/postdocs/isem/phylo/phylogenies/tools"
#Path to run file (group, id)
RUN_FILE="/home/arthur/Bureau/postdocs/isem/phylo/phylogenies/run_list"
MOM_FILE="/home/arthur/Bureau/postdocs/isem/phylo/phylogenies/mother_daughter_list"
...
rule mother_phase_run:
	input:
		vcf="{PATH}/{ID}-{REF}{EXT}bwa{EXT2}freebayes.vcf",
		momvcf=lambda wildcards: expand("{PATH}/{ID}-{REF}{EXT}bwa{EXT2}freebayes.vcf", PATH=wildcards.PATH, ID=MOMDIC[wildcards.ID], REF=wildcards.REF, EXT=wildcards.EXT, EXT2=wildcards.EXT2),
		covbed="{PATH}/{ID}-{REF}{EXT}bwa{EXT2}coverage.bed",
	...
	output:
		...
		vcfgz=temp("{PATH}/{ID}-{REF}{EXT}bwa{EXT2}freebayes.vcf.gz"),
		vcfindex=temp("{PATH}/{ID}-{REF}{EXT}bwa{EXT2}freebayes.vcf.gz.csi"),
		momvcfgz=temp("{PATH}/{ID}-mom-{REF}{EXT}bwa{EXT2}freebayes.vcf.gz"),
		momvcfindex=temp("{PATH}/{ID}-mom-{REF}{EXT}bwa{EXT2}freebayes.vcf.gz.csi"),
	...
	shell:
		...
		"bgzip -c -f {input.vcf} > {output.vcfgz}; bcftools index -f {output.vcfgz};"
		"bgzip -c -f {input.momvcf} > {output.momvcfgz}; bcftools index -f {output.momvcfgz};"
		"bcftools merge -0 -m all {output.momvcfgz} {output.vcfgz} | bcftools norm -f {input.ref} > {output.totvcf};"
